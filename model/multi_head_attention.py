...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.heads = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_dim) for i in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        concat = torch.concatenate(head_outputs, dim=-1)
        return torch.round(self.output(concat), decimals=4)
